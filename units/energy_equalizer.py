...
class Unit(unit.Unit):
    def __init__ (self, context, unit_config= None, input_map_id_list = None): #the data schema of the input map is checked by this Unit
        ...
        self.just_filter = self.getConfigOrDefault('just_filter', False)
        ...
        self.kernel_size = self.getConfigOrDefault('kernel_size', 5)
        ...
        self.contrast_curve_step = self.getConfigOrDefault('contrast_curve_step', (float(0.0), float(0.3334), float(0.6667), float(1.0)))
        self.contrast_curve_step = tuple([float(self.contrast_curve_step[i]) for i in range(len( self.contrast_curve_step))])
        self.double_inverse = self.getConfigOrDefault('double_inverse', False)
        ...
        if self.df_list is not None and len(self.df_list) > 0:
            for df in self.df_list:
                for index, row in df.iterrows():
                    ...
                    if not self.just_filter:
                        fft_original = utils.fourierTransform(img)
                        fft =  energy_equalizer.equalizationFilter(fft_original, self.direction_pass, self.direction_filter, self.ponderation_distance_factor, self.symmetry, self.kernel_size, self.std_x, self.std_y, self.square_proportion, self.double_inverse)
                        #fft = self.equalizationFilter(fft_original, self.direction_pass, self.direction_filter, self.ponderation_distance_factor, self.symmetry, self.kernel_size)

                        #print(fft)
                        equalized = np.abs(utils.inv_FFT_all_channel(fft))

                    else:
                        gaussian = self.getGaussianFilter(img.shape)
                        square = self.getSquareFilter(img.shape)
                        filter = self.filterPonderation(gaussian, square, self.direction_pass, self.ponderation_distance_factor, self.symmetry)
                        equalized, fft_filtered = self.filterImage(img, filter) 
                    ...
                    #filter = self.filterPonderation(gaussian, square, self.direction_pass, self.ponderation_distance_factor, self.symmetry)
                    #inv_filter = np.fft.ifftshift(np.abs(utils.inv_FFT_all_channel(filter))) #check the concolutional filter...
                    
                    #equalized = (equalized/np.max(equalized)*255).astype(np.uint8)
                    contrasted = energy_equalizer.contrastTunning(equalized, self.contrast_curve_step)#.astype(np.uint8)
                    if self.normalization_value != 0:
                        contrasted = (contrasted * self.normalization_value).astype(np.uint8)
                    ...
                    if self.write_output_to_disk:
                        ...
                        imageio.imwrite(uri=path, im=contrasted)  
                        #imageio.imwrite(uri=path+".png", im=equalized)  

                    else:
                        #img_filtered, fft_filtered = self.filterImage(img, filter) 
                        fft_contrasted= utils.fourierTransform(contrasted)
                    
                        the_figures2 = [utils.InputPFF(contrasted, "contrasted"), utils.InputPFF(img, row[self.image], 0, 0),  utils.InputPFF(equalized/np.max(equalized), "equalized", 0, 0)]
                        the_figures3 = [utils.InputPFF(utils.fft_img_values(np.abs(fft_contrasted)), "fft contrasted"), utils.InputPFF(utils.fft_img_values(np.abs(fft)), "equalized fft"),  utils.InputPFF(utils.fft_img_values(np.abs(fft_original)), "original fft")]

                        utils.plot_figures(the_figures2)
                        utils.plot_figures(the_figures3)

            #self.output = [1,2,3]
            self.output = i.ImageList() #missing schema output for images from this unit, might be the row dataframe with the processed image to keep track of the original image meta data
            return True
        ...
        for a in range(shape[0]):
            for s in range(shape[1]):
                ...
                if filter_value < 1:
                    ...
                    #        inverse +=  1
                    #        inverse2 += 1 / energy
                    if inverse2 != 0 and inverse != 0:
                        if self.double_inverse: 
                            inverse = inverse / inverse2
                        
                        else:
                            inverse = len(sl) / inverse
